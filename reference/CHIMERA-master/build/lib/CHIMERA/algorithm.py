...
import pickle
import optimization_utils as opu
...
def optimize(x,xd,xs,y,yd,ys,config):
    ...
    """
    params = opu.initialization(x,y,config['K'])
    ...
    while iteration < config['max_iter']:
        ...
        P = opu.Estep(y,yd,ys,tx,xd,xs,params['sigsq'],config['r'],config['rs'])
        params = opu.Mstep(y,yd,ys,x,tx,xd,xs,P,params,config)
    
        #> calculate objective function
        obj[iteration+1] = opu.calc_obj(x,y,xd,yd,xs,ys,P,params,config)
